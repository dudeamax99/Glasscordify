...
let originalFs;
try{
	originalFs = require("original-originalFs");
}catch(e){
	originalFs = require("originalFs");
}
...
class Utils{
	...
	static linkGlasscordToAppPath(_appPath, shim = undefined){
		...
		if(typeof shim !== "undefined"){
			...
			originalFs.writeFileSync(linkPath, shim.toString().replace("{GLASSCORD_ASAR_LOCATION}", "./glasscord.asar"));
		}
		return linkPath;
	}
