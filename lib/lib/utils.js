...
class Utils{
	...
	static linkGlasscordToAppPath(_appPath, shim = undefined){
		...
		if(fs.existsSync(linkPath))
			fs.unlinkSync(linkPath); // YEETUS

		if(process.platform === "win32") // DUMB SHIT INCOMING
			fs.copyFileSync(glasscordPath, linkPath);
		else
			fs.symlinkSync(glasscordPath, linkPath);

		
		if(typeof shim !== "undefined"){
			linkPath = path.resolve(_appPath, "glasscord_shim.js");
			fs.writeFileSync(linkPath, shim.toString().replace("{GLASSCORD_ASAR_LOCATION}", "./glasscord.asar"));
		}
		return linkPath;
	}
	...
	static determineMainFile(_appPath){
		if(fs.existsSync(path.resolve(_appPath, "package.json")))
	...
	static __determineMainFile(_packageJson){
		...
		try{
			...
			if(fs.existsSync(main) && fs.lstatSync(main).isDirectory()){
				// If main is a directory then we look for package.json and we'll call this function again
				if(fs.existsSync(path.resolve(main, "package.json")))
				...
				if(fs.existsSync(path.resolve(main, "index.js")))
			...
			if(path.extname(main) === ".js" && fs.existsSync(main))
				return main;
	...
	static getResourcesPath(_appRootPath){
		if(fs.existsSync(path.resolve(_appRootPath, "resources")))
